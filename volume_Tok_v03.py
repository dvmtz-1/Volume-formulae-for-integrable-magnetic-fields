# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
# 'volume_Tok_v03.py' - repurposed 'volume_1r_v05.py' to Tokamak example
#  Script to compute the first return time T as described in 'VolumeTori2-1.pdf' [9May]
#  for the axisymetric example field on the Sec 3.2 of the Burby et al [2023].
#  in cilindrical coordinates.
#  Extra: compute volume V using Theorem 1.
#
# v3 : Goals: Compute T for different y0's taken from equal spaced \psi's
#      Using routines from 'colume_1r_v06.py'
#      Extra goal: compute V by Thm 1.
# - - - - - - - - - - - - - - - - - - - - - -
#
# Input: none
#
...
pnt_list = ener_srch2(pnt_list0[0],pnt_list0[1],NPsi)
#print(pnt_list)
...
######################################################
NPnt=20  #Number of points in the Poincare Section
######################################################
...
#     print('|| N1 = ',N1)
    ...
    # Ensuring a minimum of poloidal crossings
    while N1< NPnt:
    ...
    #th0 = np.arctan2(X0[2], X0[0]-1)
    th0 = atan3(X0[2], X0[0]-1)
#     jj = findcrossings2(Xc,th0,X0[0])
#     print('T_j = ', jj, t[jj],' | th -th0 = ',np.arctan2(Xc[jj][2],Xc[jj][0]) - th0, np.arctan2(Xc[jj+1][2],Xc[jj+1][0])- th0)


    ind_int = findcrossings3(Xc,th0,X0[0])
#     print('ind_int = ',ind_int)

    # tt2 = 0
    kk=0
    T0 = 0
#     print('               Energy       j          T                DT               th-th0')
    # print('   Energy     j          t_j              t_cross          th-th0')
    for j in ind_int:
        tt2 = 0
        ...
        while j0 > len(t):
            ...
            #print(tt2)
        tcross = tt2 + t[j0] #+dt3
    #     print('j = ', j,'Kc =', m1*Kc[1] - n1*Kc[2], ' tcross = ', tcross, 'dt3 = ', dt3 )
    #     print('j = ', j, ' |  t_j =', tcross)
        #print('[',kk,'] ',energy(x0,z0), j, tcross, tcross + dt3 ,(np.arctan2(Kc[2], Kc[0]) -th0))
        #print('[',kk,'] ',energy(x0,z0), j, tcross + dt3 , tcross + dt3 - T0,(np.arctan2(Kc[2], Kc[0]-1) -th0),Kc[0]-1)
#         print('[',"{:.4f}".format(x0),'] ',"{:.5f}".format(energy(x0,z0)), j, "{:.5f}".format(tcross + dt3 - T0),Kc[0]-1)
        T0 = tcross + dt3
        kk += 1
        if kk > 4:
            break
        if kk == 1:
            AvrT = AvrT +  (tcross + dt3) / (NPsi+1)
            break
